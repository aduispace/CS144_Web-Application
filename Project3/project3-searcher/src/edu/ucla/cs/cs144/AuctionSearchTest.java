...
public class AuctionSearchTest {
	public static void main(String[] args1)
	{
		...
		SearchRegion region =
		    new SearchRegion(0, -118.63, 34.201, -117.38); 
		SearchResult[] spatialResults = as.spatialSearch("camera", region, 0, 20);
		System.out.println("Spatial Seacrh");
		System.out.println("Received " + spatialResults.length + " spatial results ");
		...
		region = new SearchRegion(180, 180, -180, -180);
		spatialResults = as.spatialSearch("superman", region, 0, 100);
		for(SearchResult result : spatialResults) {
			System.out.println(result.getItemId() + ": " + result.getName());
		}
	}
